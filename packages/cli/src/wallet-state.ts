// Wallet sync-state persistence. Mirrors the format used by midnight-app;
// no SDK imports, all I/O via function parameters.
//
// Why: without persistence, every `npm run deploy` rebuilds each child wallet
// from seed and re-syncs against the chain. On public networks (preview,
// preprod) that's minutes per run — and painful on retries after a transient
// failure. The SDK exposes serializeState() and restore() on each child wallet
// class; wallet.ts is the glue that uses them, and this file is the on-disk
// format underneath.
